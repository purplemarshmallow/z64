    ...
    if (dxt == 0) {
        ...
        for (i = 0; i < width / 4; i++) {
            ...
            if (src_base >= rdram_in_bytes / sizeof(src[0])) {
                LOGERROR("ERROR:  Block load address 0x%lX exceeds 8 MB DRAM.\n", src_base);
                return;
            }
...
static void rdp_load_tile(uint32_t w1, uint32_t w2)
{
    ...
    case RDP_PIXEL_SIZE_32BIT:
    {
        ...
        {
            LOGERROR("rdp_load_tile 32-bit: tmem %04X, width %d, height %d = %d\n", rdpTiles[tilenum].tmem, width, height, width*height);
        }
    ...
    default:	LOGERROR("RDP: load_tile: size = %d\n", rdpTiSize);
    }
}
